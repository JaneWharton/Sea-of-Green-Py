...
# unique const  :      char,      fgcol,     bgcol, enter,leave,opaque,damp
    TILE_OCEAN  : Tile(TILE_OCEAN,'neutral', 'deep',  100,0,    False, 1,),
    TILE_AIR    : Tile(TILE_AIR,  'purple',  'deep',  100,0,    False, 1,),
    TILE_WALL   : Tile(TILE_WALL, 'dkred',   'orange',0,  100,  True,  50),
    }
...
##        self.levels = {}
    ...
    def init_terrain(self, default=TILE_OCEAN):
        '''
...
##                    print("color is ", self.get_color(x, y))
##                    print("bgcolor is ", self.get_bgcolor(x, y))
    ...
    def _apply_rendered_bgcol(self, x, y, ent):
        ...
        '''
        bgTile=self.get_bgcolor(x, y) # from terrain itself
        bgCol=bgTile
        if self.nthings(x, y) >= 2:
            ...
            bgCol=rog.world().component_for_entity(ent, cmp.Image).bgcol
