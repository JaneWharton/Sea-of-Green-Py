...
def _getnwalls(xx, yy, z, width, height):
    walls = 0
    if (xx-1 > 0 and rog.getmap(z).tileat(xx-1, yy) == TILE_WALL):
        walls += 1
    if (xx+1 < width-1 and rog.getmap(z).tileat(xx+1, yy) == TILE_WALL):
        walls += 1
    if (yy-1 > 0 and rog.getmap(z).tileat(xx, yy-1) == TILE_WALL):
        walls += 1
    if (yy+1 < height-1 and rog.getmap(z).tileat(xx, yy+1) == TILE_WALL):
        walls += 1
    return walls
    ...
    if diglist:
        ...
        for tile in diglist:
            ...
            if not rog.is_in_grid(x,y):
                continue
            rog.getmap(rog.dlvl()).tile_change(x,y, TILE_OCEAN)
        ...
        for tile in room.area:
            ...
            if not rog.is_in_grid(xi,yi):
                continue
            rog.getmap(rog.dlvl()).tile_change(xi, yi, TILE_OCEAN)
...
def _generate_level_tree(width, height, z, density=400, maxDensity=2000):
    ...
    for tile in origin.area:
        ...
        if not rog.is_in_grid(xx,yy):
            continue
        rog.getmap(z).tile_change(xx, yy, TILE_OCEAN)
    ...
    while list_tiles:
        for tile in list_tiles:
            ...
            # fill in dead ends
            if rog.getmap(z).tileat(xx, yy) == TILE_OCEAN:
...
##                    print("dead end found at {}, {}".format(xx, yy))
                    ...
                    if yy+1 < height-1:
                        new_list.append((xx, yy+1,))
                    rog.getmap(z).tile_change(xx, yy, TILE_WALL)
        list_tiles=new_list
        new_list=[]
    #
